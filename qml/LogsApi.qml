    ...
    function getLogsChanges()
    {
        dataBase.transaction(
                    function(tx)
                    {
                        ...
                        {
                            let list=[]
                            let result = tx.executeSql("SELECT record_id FROM ServerChanges WHERE table_id = 7 AND changes_type !=3 AND user_id = ? GROUP BY record_id",currentUser.id)
                            ...
                            result = tx.executeSql("    SELECT record_id,id FROM ServerChanges WHERE table_id = 7 AND changes_type  =3 AND user_id = ? GROUP BY record_id",currentUser.id)
                            if(result.rows.length > 0)
                            {
                    ...
                    if(response.ok)
                    {
                        if(response.code === 201){
                            if(local_id === null)
                            {
                                ...
                                localDB.insertDeviceChanges([{"table_id":7,   "record_id":response.result.id,    "changes_type":1,  "user_id":currentUser.id}])
                            }
                            else{
                    ...
                    if(response.ok)
                    {
                        if(response.code === 200){
                            if(local_id === null)        {
                                ...
                                localDB.insertDeviceChanges([{"table_id":7,   "record_id":response.result.id,    "changes_type":2,  "user_id":currentUser.id}])
                    ...
                    if(response.ok)
                    {
                        if(response.code === 200){
                            ...
                            if(local_id === null)
                            {
                                ...
                                localDB.insertDeviceChanges([{"table_id":7,   "record_id":logId,    "changes_type":3,  "user_id":currentUser.id}])
                            }
                            else{
                                localDB.deleteFromLocalChanges(change_id)
                            }
