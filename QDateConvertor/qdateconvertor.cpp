...
QDateConvertor::QDateConvertor(QQuickItem *parent)
    : QQuickItem(parent)
{
...
bool QDateConvertor::is_leap(int year)
{
    ...
    {
        if (mod == 1 || mod == 5 || mod ==9 || mod == 13 || mod == 17 || mod==21 || mod==26 ||mod==23)
        {
            return true;
        }
        else{
...
QStringList QDateConvertor::get_month()
{
    this->set_months();
    QStringList list;
    list.append(Month.values());
    return list;
}
